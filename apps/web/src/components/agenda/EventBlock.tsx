 ...
 */
export function EventBlock({ event, color, compact, style, onPointerDown, onResizeStart, onSelect }: EventBlockProps) {
  const declined = event.selfResponse === "declined";
  const awaiting = event.selfResponse === "needsAction";
  return (
    <button
      type="button"
      aria-label={`${event.summary}, ${formatSpan(event)}`}
    ...
    >
      <span className="flex min-w-0 items-center gap-1">
        <span className={`truncate font-medium ${declined ? "line-through" : ""}`}>{event.summary}</span>

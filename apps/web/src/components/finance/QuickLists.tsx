...
import type { Account, Asset, Goal } from "./fixtures";
...
export function AccountsList({ accounts }: AccountsListProps) {
  ...
  return (
    ...
    >
      ...
      {top5.length === 0 ? (
        <p className="text-xs" style={{ color: "var(--text-muted)" }}>Aucun compte enregistré.</p>
      ) : (
...
export function AssetsList({ assets }: AssetsListProps) {
  ...
  return (
    ...
    >
      ...
      {assets.length === 0 ? (
        <p className="text-xs" style={{ color: "var(--text-muted)" }}>Aucun actif enregistré.</p>
      ) : (
        <div className="flex flex-col gap-2">
          {Object.entries(byCategory).map(([cat, catAssets]) => {
            const total = catAssets.reduce((s, a) => s + a.currentValue, 0);
            return (
              <div key={cat} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                      {catAssets.length} actif{catAssets.length > 1 ? "s" : ""}
                    </p>
                  </div>
                </div>
                <span className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>
...
export function GoalsList({ goals }: GoalsListProps) {
  ...
  return (
    ...
    >
      ...
      {top3.length === 0 ? (
        <p className="text-xs" style={{ color: "var(--text-muted)" }}>Aucun objectif enregistré.</p>
      ) : (

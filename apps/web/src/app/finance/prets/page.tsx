      ...
      <table className="w-full text-xs">
        <thead>
          <tr style={{ backgroundColor: "var(--surface-2)" }}>
            {["#", "Date", "Mensualité", "Capital", "Intérêts", "Restant"].map((h) => (
              <th
                key={h}
        ...
        <tbody>
          {first12.map((row) => (
            ...
            >
              ...
              <td className="px-3 py-2 font-medium tabular-nums" style={{ color: "var(--text-primary)" }}>
                {formatCurrency(row.payment)}
              </td>
              <td className="px-3 py-2 tabular-nums" style={{ color: "var(--success)" }}>
                {formatCurrency(row.principal)}
              </td>
              <td className="px-3 py-2 tabular-nums" style={{ color: "var(--danger)" }}>
...
function LoanRow({ loan }: { loan: Loan }) {
  ...
  const paidPct = ((loan.principal - remaining) / loan.principal) * 100;
    ...
    <div className="overflow-hidden rounded-xl border" style={{ borderColor: "var(--border-subtle)" }}>
      ...
      >
        ...
        <div className="text-right">
          ...
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>
            restant / {formatCurrency(loan.principal)}
          </p>
        </div>
        <div className="text-right">
          ...
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>
            fin {formatDate(endDate.toISOString())}
          </p>
        </div>
        <div className="w-24">
          ...
          >
            <div
              ...
              style={{ width: `${paidPct}%`, backgroundColor: color }}
            />
          </div>
        </div>
      </div>
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      >
        ...
        <p className="mt-1 text-2xl font-bold" style={{ color: "var(--danger)" }}>
          {formatCurrency(
            LOANS.reduce((s, l) => s + getLoanRemainingPrincipal(l), 0)
          )}
        </p>
      </div>
    </div>
